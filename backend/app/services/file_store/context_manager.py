...
class FileStager:
    ...
    def __init__(
        self,
        filename,
        temp_file_content: str | bytes | None = None,
    ):
        self.filename: str = filename
        self.temp_file = None
        self.temp_file_path: Path | None = None
        self.temp_file_content: str | bytes | None = temp_file_content

    def __enter__(self) -> Path:
        self.temp_file = NamedTemporaryFile(
            delete=False, suffix=Path(self.filename).suffix
        )
        self.temp_file_path = Path(self.temp_file.name)
        with self.temp_file as file:
            file.write(
                self.temp_file_content.encode("utf-8")
                if isinstance(self.temp_file_content, str)
                else self.temp_file_content
            )
